...
- [ ] Create `src/lib/repositories/ExpenseRepository.ts`
...
- [ ] Create `src/lib/repositories/MockExpenseRepository.ts`
...
- [ ] Wrap `App` (in `src/main.tsx`) with `<RepositoryProvider>` inside `<BrowserRouter>` but outside `<AuthProvider>` (or after, depending on structure — just ensure it's a top-level provider)
...
- [ ] Write `src/lib/repositories/MockExpenseRepository.test.ts`
...
- [ ] Write `src/context/RepositoryContext.test.tsx`
...
- [ ] Update `docs/decisions/architecture/0010-mock-repository-pattern.md`:
...
- [ ] Repository interface is defined and clearly typed
- [ ] `MockRepository` loads mock expenses into in-memory Map on construction
- [ ] All mutations return new `Expense` objects, never mutate originals
- [ ] `reset()` method works correctly for test setup
- [ ] `useRepository()` hook is available and can be called from components
- [ ] All unit tests pass
- [ ] Architecture docs are updated to remove "PLANNED" status
- [ ] ADR-0010 status is updated to "Implemented"
