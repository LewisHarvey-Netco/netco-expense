...
- [x] Update `ExpenseDetailPage.tsx` to include ReviewDecisionForm in right column
- [x] Add local state for the current expense (loaded from mock on mount)
- [x] When ReviewDecisionForm submits:
...
- [x] After submission, disable the form (or navigate away, or show "Decision recorded")
- [x] Handle edge cases:
...
- [x] Validate status transitions follow the workflow (Submitted → Approved or Changes Requested, etc.)
...
- [x] No architecture docs updates needed for this task (repository pattern was documented in task 08a)
